...
/**
 * A context to be notified by `scala.debug.concurrent.blocking` when
 * a thread is about to block. In effect this trait provides
 * the implementation for `scala.debug.concurrent.Await`.
 * `scala.debug.concurrent.Await.result()` and `scala.debug.concurrent.Await.ready()`
 ...
 *    // for scala.debug.concurrent.blocking
 *  }
 *  }}}
 */
trait BlockContext {
   ...
   *
   * Clients must use `scala.debug.concurrent.blocking` or `scala.debug.concurrent.Await` instead.
   */
