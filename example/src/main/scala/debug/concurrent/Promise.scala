 ...
 *  Note: Using this method may result in non-deterministic debug.concurrent programs.
 */
trait Promise[T] {
...
object Promise {
  ...
  def fromTry[T](result: Try[T]): Promise[T] = {
    example.log(s"${Thread.currentThread.getName} Creating a new Kept Promise with $result", Level.INTERNAL)
