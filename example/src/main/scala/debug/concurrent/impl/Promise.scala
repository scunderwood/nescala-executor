...
import debug.concurrent.{CanAwait, ExecutionContext, ExecutionException, OnCompleteRunnable, TimeoutException}
import debug.concurrent.Future.InternalCallbackExecutor
import debug.concurrent.duration.{Duration, FiniteDuration}
...
private[concurrent] trait Promise[T] extends debug.concurrent.Promise[T] with debug.concurrent.Future[T] {
...
private[concurrent] object Promise {
  ...
  private def resolveTry[T](source: Try[T]): Try[T] = {
    source match {
      case Failure(t) =>
        ...
        resolver(t)
      case _ =>
        example.log(s"Promise's Try didn't fail: Returning $source", Level.INTERNAL)
        source
    }
  }
  ...
  private def resolver[T](throwable: Throwable): Try[T] = throwable match {
    ...
    *
    * Inspired by: http://gee.cs.oswego.edu/cgi-bin/viewcvs.cgi/jsr166/src/main/java/util/debug.concurrent/locks/AbstractQueuedSynchronizer.java
  ...
  class DefaultPromise[T] extends AtomicReference[AnyRef](Nil) with Promise[T] {
    ...
    final def tryComplete(value: Try[T]): Boolean = {
      example.log("In Default promise tryComplete", Level.INTERNAL)
      val resolved = resolveTry(value)
      tryCompleteAndGetListeners(resolved) match {
        case null             =>
          example.log("Matched null, returning false", Level.INTERNAL)
          false
        case rs if rs.isEmpty =>
          example.log("No callback runables, returning True", Level.INTERNAL)
          true
        case rs               =>
  ...
  object KeptPromise {
    ...
    private[this] sealed trait Kept[T] extends Promise[T] {
      ...
      override def tryComplete(value: Try[T]): Boolean = {
        example.log("Kept Promise Try Complete", Level.INTERNAL)
        false
      }
    ...
    def apply[T](result: Try[T]): debug.concurrent.Promise[T] =
